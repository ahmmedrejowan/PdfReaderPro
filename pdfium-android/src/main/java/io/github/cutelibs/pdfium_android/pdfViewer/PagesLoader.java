...
import static io.github.cutelibs.pdfium_android.pdfViewer.util.Constants.Cache.CACHE_SIZE;
import static io.github.cutelibs.pdfium_android.pdfViewer.util.Constants.PRELOAD_OFFSET;
...
class PagesLoader {
    ...
    private boolean loadCell(int page, int row, int col, float pageRelativePartWidth, float pageRelativePartHeight) {
        ...
        if (renderWidth > 0 && renderHeight > 0) {
            if (!pdfView.cacheManager.upPartIfContained(page, pageRelativeBounds, cacheOrder)) {
                pdfView.renderingHandler.addRenderingTask(page, renderWidth, renderHeight, pageRelativeBounds, false, cacheOrder, pdfView.isBestQuality(), pdfView.isAnnotationRendering());
            }
    ...
    private void loadThumbnail(int page) {
        ...
        if (!pdfView.cacheManager.containsThumbnail(page, thumbnailRect)) {
            pdfView.renderingHandler.addRenderingTask(page, thumbnailWidth, thumbnailHeight, thumbnailRect, true, 0, pdfView.isBestQuality(), pdfView.isAnnotationRendering());
        }
    }
